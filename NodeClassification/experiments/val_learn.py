...
class Our_Trainer:
    ...
    def train(self):
        ...
        for epoch in range(1, self.args.epochs + 1):
            ...
            scale_list.append(torch.mean(self.t).cpu().detach().numpy().tolist())
            scale_list_max.append(self.t[999].cpu().detach().numpy().tolist())
            scale_list_min.append(self.t[125].cpu().detach().numpy().tolist())
            
            a = torch.ones_like(heat_kernel)-torch.eye(heat_kernel.shape[0]).to(self.device)
            h = torch.mul(heat_kernel, a.to(self.device))
            heat_kernel_list.append(torch.mean(h).cpu().detach().numpy().tolist())

            #heat_kernel_list.append(torch.mean(heat_kernel).cpu().detach().numpy().tolist())
        ...
        return scale_list, scale_list_max, scale_list_min, heat_kernel_list
...
class Our_APPROX_Trainer:
    ...
    def train(self):
        ...
        return scale_list_max, scale_list_min, heat_kernel_list
...
def main():
    ...
    device0 = torch.device('cuda:4' if torch.cuda.is_available() else 'cpu')
    ...
    device3 = torch.device('cuda:7' if torch.cuda.is_available() else 'cpu')
    ...
    scale_list1, scale_list1_max, scale_list1_min, heat_kernel_list1 = trainer_ours.train()
    #trainer_ours.test()
    # print(scale_list1.index(max(scale_list1)))
    # print(max(scale_list1))
    # print(scale_list1.index(min(scale_list1)))
    # print(min(scale_list1))
    # sys.exit(0)
    ...
    scale_list2_max, scale_list2_min, heat_kernel_list2 = trainer_ours_approx0.train()
    trainer_ours_approx0.test()
    ...
    scale_list3_max, scale_list3_min, heat_kernel_list3 = trainer_ours_approx1.train()
    #trainer_ours_approx0.test()
    ...
    scale_list4_max, scale_list4_min, heat_kernel_list4 = trainer_ours_approx2.train()
    #trainer_ours_approx0.test()
    # print(heat_kernel_list1)
    # print(heat_kernel_list2)
    # print(heat_kernel_list3)
    # print(heat_kernel_list4)
    plt.figure(figsize=(12, 4))
    ax1 = plt.subplot(1, 2, 1)
    #plt.subplots_adjust(wspace=0.8)
    #plt.title('Pubmed', fontweight='bold', fontsize=22, pad=20)
    plt.plot(scale_list1_max, linewidth = 3, color='blue', linestyle='solid', label='[a]')
    plt.plot(scale_list1_min, linewidth = 3, color='blue', linestyle='solid', label='[a]')
    plt.plot(scale_list2_max, linewidth = 3, color='orange', linestyle='dashed', label='[b]')
    plt.plot(scale_list2_min, linewidth = 3, color='orange', linestyle='dashed', label='[b]')
    plt.plot(scale_list3_max, linewidth = 3, color='green', linestyle='dashed', label='[b]')
    plt.plot(scale_list3_min, linewidth = 3, color='green', linestyle='dashed', label='[b]')
    plt.plot(scale_list4_max, linewidth = 3, color='red', linestyle='dashed', label='[b]')
    plt.plot(scale_list4_min, linewidth = 3, color='red', linestyle='dashed', label='[b]')
    
    #plt.plot(scale_list3, linewidth = 3, linestyle='dashed', label='[c]')
    #plt.plot(scale_list4, linewidth = 3, linestyle='dashed', label='[d]')
    ...
    #plt.title(args.data, fontweight='bold', fontsize=16)
    plt.plot(heat_kernel_list1, linewidth = 3, linestyle='solid', label='[a]')
    plt.plot(heat_kernel_list2, linewidth = 3, linestyle='dashed', label='[b]')
    plt.plot(heat_kernel_list3, linewidth = 3, linestyle='dashed', label='[c]')
    plt.plot(heat_kernel_list4, linewidth = 3, linestyle='dashed', label='[d]')
    plt.xticks(fontsize=10, fontweight='bold')
    plt.yticks(fontsize=10, fontweight='bold')
    plt.xlabel('Epoch', fontsize=12, fontweight='bold')
    plt.ylabel('Heat Kernel', fontsize=12, fontweight='bold')
    #plt.xlabel('Epoch')
    ...
    plt.show()
    plt.savefig(args.data + '_fig.png')
