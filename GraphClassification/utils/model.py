...
def select_model(args, num_ROI_features, num_used_features, adjacencies, labels):
    if args.model == 'svm':
        ...
        if args.layer_num == 1:
            model = ASAP1(adj_dim=num_ROI_features,
                        in_dim=num_used_features,
                        hid_dim=args.hidden_units,
                        out_dim=torch.max(labels).item() + 1,
                        dropout=args.dropout_rate)
        elif args.layer_num == 2:
            model = ASAP2(adj_dim=num_ROI_features,
                        in_dim=num_used_features,
                        hid_dim=args.hidden_units,
                        out_dim=torch.max(labels).item() + 1,
                        dropout=args.dropout_rate)
        elif args.layer_num == 3:
            model = ASAP3(adj_dim=num_ROI_features,
                        in_dim=num_used_features,
                        hid_dim=args.hidden_units,
                        out_dim=torch.max(labels).item() + 1,
                        dropout=args.dropout_rate)
        elif args.layer_num == 4:
            model = ASAP4(adj_dim=num_ROI_features,
                        in_dim=num_used_features,
                        hid_dim=args.hidden_units,
                        out_dim=torch.max(labels).item() + 1,
                        dropout=args.dropout_rate)
...
def select_trainer(args, device, model, optimizer, data_loader_train, data_loader_test, adjacencies):
    if args.model == 'svm':
        ...
        trainer = ASAP_Trainer(args, device, model, optimizer, data_loader_train, data_loader_test, data_loader_test, adjacencies[0].shape[0])
