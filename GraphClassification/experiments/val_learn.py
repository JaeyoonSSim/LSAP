        ...
        for iter in range(1, self.args.epochs + 1):
            ...
            for adjacency, feature, label, eigenvalue, eigenvector, laplacian in self.train_loader:
                ...
                if self.poly == 0: # Chebyshev
                    eigmax = eigenvalue[:,eigenvalue.shape[1]-1] # Maximum value of eigenvalue
                    heat_kernel, heat_kernel_grad = compute_heat_kernel_chebyshev(laplacian, self.m_che, self.t, eigmax, self.device)
...
def main():
    ...
    if torch.cuda.is_available() and args.model != 'svm':
        ...
        adjacencies_h = adjacencies_l.to(device) # Shape: (# subjects, # ROI feature, # ROI features)
        features_h = features_l.to(device) # Shape: (# subjects, # ROI features, # used features)
        labels_h = labels_l.to(device) # Shape: (# subjects)
        eigenvalues_h = eigenvalues_l.to(device) # Shape: (# subjects, # ROI feature)
        eigenvectors_h = eigenvectors_l.to(device) # Shape: (# subject, # ROI_feature, # ROI_feature)
        laplacians_h = laplacians_l.to(device)
    ...
    for i, idx_pair in enumerate(idx_pairs):
        ...
        model_e = Exact2(adj_dim=num_ROI_features,
                            in_dim=num_used_features,
                            hid_dim=args.hidden_units,
                            out_dim=torch.max(labels0).item() + 1,
                            dropout=args.dropout_rate)
        model_c = ASAP2(adj_dim=num_ROI_features,
                            in_dim=num_used_features,
                            hid_dim=args.hidden_units,
                            out_dim=torch.max(labels_c).item() + 1,
                            dropout=args.dropout_rate)
        model_h = ASAP2(adj_dim=num_ROI_features,
                            in_dim=num_used_features,
                            hid_dim=args.hidden_units,
                            out_dim=torch.max(labels_h).item() + 1,
                            dropout=args.dropout_rate)
        model_l = ASAP2(adj_dim=num_ROI_features,
                            in_dim=num_used_features,
                            hid_dim=args.hidden_units,
        ...
        trainer_e = Our_Trainer(args, device, model_e, data_loader_train_e, data_loader_test_e, data_loader_test_e, adjacencies0[0].shape[0])
        s_e_max, s_e_min = trainer_e.train()
        ...
        s_c_max, s_c_min = trainer_chl0.train()
        ...
        s_h_max, s_h_min = trainer_chl1.train()
        ...
        s_l_max, s_l_min = trainer_chl2.train()
    ...
    plt.plot(s_e_max, linewidth = 3, color='blue', linestyle='solid', label='[a]')
    plt.plot(s_e_min, linewidth = 3, color='blue', linestyle='solid', label='[a]')
    plt.plot(s_c_max, linewidth = 3, color='orange', linestyle='dashed', label='[b]')
    plt.plot(s_c_min, linewidth = 3, color='orange', linestyle='dashed', label='[b]')
    plt.plot(s_h_max, linewidth = 3, color='green', linestyle='dashed', label='[b]')
    plt.plot(s_h_min, linewidth = 3, color='green', linestyle='dashed', label='[b]')
    plt.plot(s_l_max, linewidth = 3, color='red', linestyle='dashed', label='[b]')
    plt.plot(s_l_min, linewidth = 3, color='red', linestyle='dashed', label='[b]')
    ...
    plt.show()
    plt.savefig(str(args.epochs) + '_fig.png')
