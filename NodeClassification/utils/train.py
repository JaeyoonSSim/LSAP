...
class Exact_Trainer:
    ...
    def test(self):
        ...
        #if self.acc_max > accuracy_test:
        #    output = self.acc_max
        #else:
        #    output = accuracy_test.detach().cpu()
        return accuracy_test.detach().cpu()
...
class Our_APPROX_w_Trainer:
    def __init__(self, args, device, model, optimizer, A, X, y, eigenvalue, eigenvector, laplacian, idx_train, idx_val, idx_test, adj_size):
        ...
        if args.use_t_local == 1: # Local scale
            #self.t = torch.empty(adj_size).fill_(2.)
            self.t = Variable(torch.empty(adj_size).fill_(2.))
        else: # Global scale
            self.t = torch.tensor([2.]) 
    ...
    def t_loss(self, t):
        ...
        t_l = torch.where(t < self.t_loss_threshold, t_one, t_zero)
    ...
    def scale_update(self, output, feature, label, heat_kernel, heat_kernel_grad):
        ...
        if self.args.layer_num == 1:
            ...
            dl_dc = torch.mul(dl_dh, self.model.gcn.rdp2) # (# node, # hidden unit)
            dl_dc_first = torch.mul((dl_dc @ self.model.gcn.f.swapaxes(0, 1)), heat_kernel_grad) # (# node, # node)
            dl_dc_second = torch.matmul(torch.mul(torch.mul(dl_dc, self.model.gcn.rdp) 
                            @ feature.swapaxes(0, 1), heat_kernel_grad), 
                            heat_kernel.swapaxes(0, 1)) # (# node, # node)
            # import pdb
            # pdb.set_trace()
            dl_dt = dl_dc_first + dl_dc_second # (# node, # node)
    ...
    def train(self):
        ...
        for epoch in range(1, self.args.epochs + 1):
            ...
            # if self.args.polynomial == 0: # Chebyshev
            #     heat_kernel, heat_kernel_grad = compute_heat_kernel_chebyshev(T_n, self.m_che, self.t, b, self.device, self.args.hk_threshold)
            # elif self.args.polynomial == 1: # Hermite
            ...
            # Use heat kernel instead of adjacency matrix
            output, t = self.model.forward(self.X, P_n, b) # Shape: (1, # node, # feature), (1, # node, # node) -> (# node, # class)
            
            loss_train = F.nll_loss(output[self.idx_train], self.y[self.idx_train]) + self.t_loss(t)
            #loss_train = self.t_loss(t)
            ...
            #elf.optimizer.step() # Updates the parameters
    ...
    def test(self):
        ...
        if self.args.polynomial == 0: # Chebyshev
            b = self.eigenvalue[-1]
            T_n = compute_Tn(self.laplacian, self.m_che, b, self.device)
            heat_kernel, _ = compute_heat_kernel_chebyshev(T_n, self.m_che, self.t, b, self.device, self.args.hk_threshold)
        elif self.args.polynomial == 1: # Hermite
            H_n = compute_Hn(self.laplacian, self.m_her, self.device)
